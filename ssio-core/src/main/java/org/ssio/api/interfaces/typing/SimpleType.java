...
public enum SimpleType {
     ...
     */
    public static SimpleType fromRealType(Class<?> realType) {
        if (realType.isEnum()) {
            return Enum;
        }

        return Arrays.stream(SimpleType.values()).filter(t -> t.getRealType().equals(realType)).findFirst().orElse(null);
    }
